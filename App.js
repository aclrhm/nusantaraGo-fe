...
export default function App() {
  ...
  const isLargeScreen = width > 768; // Deteksi layar desktop/tablet vs mobile

  // State Utama
  const [destinations, setDestinations] = useState([]);
  const [loading, setLoading] = useState(true);
  const [apiError, setApiError] = useState(false);
  const [isAdminMode, setIsAdminMode] = useState(false); // Mode Pengguna vs Admin
  ...
  // State Dropdown/Selector Modals (Mobile-friendly picker)
  const [isSortPickerOpen, setIsSortPickerOpen] = useState(false);

  // State Detail & Form Modals
  ...
  // 1. Fetch Data dari Go Backend API
  const fetchDestinations = async () => {
    ...
    try {
      ...
      setApiError(true);
      showToast('Gagal terhubung ke backend Golang. Menampilkan data lokal (fallback).', 'error');
      loadFallbackData();
    } finally {
      setLoading(false);
    }
  ...
  const loadFallbackData = () => {
    ...
    let filtered = localData.filter(d => {
      const matchCat = selectedCategory === 'semua' || d.category.toLowerCase() === selectedCategory.toLowerCase();
    ...
    if (sortBy === 'distance_asc') filtered.sort((a, b) => a.distance - b.distance);
    else if (sortBy === 'cost_asc') filtered.sort((a, b) => a.cost - b.cost);
    else if (sortBy === 'cost_desc') filtered.sort((a, b) => b.cost - a.cost);
    else if (sortBy === 'facilities_desc') filtered.sort((a, b) => b.facilities.length - a.facilities.length);

    setDestinations(filtered);
  };

  // Trigger fetch ketika query/filter berubah
  useEffect(() => {
    fetchDestinations();
  ...
  const handleAddSubmit = async () => {
    ...
    const newDest = {
      ...
      rides: formData.rides ? formData.rides.split(',').map(r => r.trim()).filter(r => r !== '') : [],
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/destinations/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id, name) => {
    // Pada web standard window.confirm disukai, pada mobile alert dialog lebih baik.
    const confirmed = Platform.OS === 'web' 
      ? window.confirm(`Apakah Anda yakin ingin menghapus destinasi "${name}"?`)
      : true; // fallback mobile
    ...
    <View style={styles.appContainer}>
      ...
      <View style={styles.header}>
        <View style={styles.brand}>
          ...
          <Text style={styles.brandName}>Nusantara<Text style={{ color: '#fff' }}>Go</Text></Text>
        </View>
        ...
        <View style={styles.headerActions}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              ...
              showToast(`Berhasil beralih ke mode ${!isAdminMode ? 'Admin' : 'Pengguna biasa'}`, 'success');
            }}
          >
          ...
          <View style={styles.offlineBanner}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.offlineText}>
                Backend Golang tidak terdeteksi aktif. Menggunakan database simulasi lokal agar Anda bisa langsung mencoba fitur.
              </Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, isLargeScreen && styles.modalContentLarge]}>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <Text style={styles.formLabel}>Fasilitas (Pisahkan dengan tanda koma)</Text>
              <TextInput 
                style={styles.formInput} 
              ...
              <Text style={styles.formLabel}>Wahana (Pisahkan dengan tanda koma)</Text>
              <TextInput 
                style={styles.formInput} 
